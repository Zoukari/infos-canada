...
const parser = new Parser({
  timeout: 10000,
  headers: { 'User-Agent': 'InfosCanada-Bot/1.0 (https://infos-canada.vercel.app; contact@infos-canada.ca)' },
...
// Parse GDELT date format "20260919T143000Z" -> Date
...
interface GdeltArticle { title?: string; url?: string; seendate?: string; domain?: string }

async function fetchGdeltItems(query: string): Promise<NormalizedItem[]> {
  const apiUrl = `https://api.gdeltproject.org/api/v2/doc/doc?query=${encodeURIComponent(query)}&mode=artlist&maxrecords=50&format=json&timespan=3d`
  const res = await fetch(apiUrl, { headers: { 'User-Agent': 'InfosCanada-Bot/1.0' } })
  if (!res.ok) throw new Error(`GDELT status ${res.status}`)
  const data = await res.json()
  const articles: GdeltArticle[] = data.articles || []
  return articles
    .filter(a => a.url && a.title)
    .map(a => ({ title: a.title as string, link: a.url as string, pubDate: a.seendate ? parseGdeltDate(a.seendate) : new Date(), description: '' }))
}

async function fetchRssItems(rssUrl: string, isGoogleNews: boolean): Promise<NormalizedItem[]> {
...
function detectProvince(text: string): string {
  ...
  if (t.includes('nouveau-brunswick') || t.includes('new brunswick') || t.includes('moncton') || t.includes('fredericton') || t.includes('dieppe') || t.includes('saint john') || t.includes('edmundston')) return 'NB'
...
function detectCategorySlug(text: string, sourceCategory?: string | null): string {
  ...
  if (t.includes('nbpnp') || t.includes('candidats du nouveau-brunswick')) return 'nbpnp'
  ...
  if (t.includes('edmundston')) return 'edmundston'
  if ((t.includes('logement') || t.includes('loyer') || t.includes('housing') || t.includes('rent')) && (t.includes('nouveau-brunswick') || t.includes('new brunswick'))) return 'logement-nb'
  if (t.includes('logement') || t.includes('loyer') || t.includes('housing') || t.includes('rent') || t.includes('immobilier')) return 'logement'
  if ((t.includes('emploi') || t.includes('travail') || t.includes('job') || t.includes('recrutement')) && (t.includes('nouveau-brunswick') || t.includes('new brunswick'))) return 'emploi-nb'
...
function generateSummary(title: string, description: string | undefined): { summary: string; whyItMatters: string } {
  ...
  } else if (t.includes('logement') || t.includes('loyer') || t.includes('rent') || t.includes('housing')) {
  ...
  } else if (t.includes('emploi') || t.includes('travail') || t.includes('recrutement') || t.includes('job')) {
    whyItMatters = 'Cette actualité intéresse les travailleurs, chercheurs d\'emploi et entreprises du Nouveau-Brunswick.'
  } else if (t.includes('santé') || t.includes('health')) {
...
export async function scrapeSource(sourceId: string): Promise<{ fetched: number; newArticles: number; error?: string }> {
  ...
  const log = await prisma.scrapingLog.create({ data: { sourceId, status: 'running' } })
  let fetched = 0
  let newArticles = 0

  try {
    if (!source.rssUrl) {
      await prisma.scrapingLog.update({ where: { id: log.id }, data: { status: 'error', finishedAt: new Date(), errorMessage: 'Pas de source configurée' } })
      return { fetched: 0, newArticles: 0, error: 'Pas de source' }
    }

    let items: NormalizedItem[]
    if (source.type === 'gdelt') {
      items = await fetchGdeltItems(source.rssUrl)
    } else {
      const isGoogleNews = source.url?.includes('news.google.com') || false
      items = await fetchRssItems(source.rssUrl, isGoogleNews)
    }
    ...
    for (const item of items) {
      ...
      if (!category) category = await prisma.category.findFirst({ where: { slug: 'actualites' } })

      const keywords = [...new Set([
        province, city, categorySlug,
      ...
      await prisma.article.create({
        data: {
          title: titleText, slug: slugify(titleText), originalTitle: titleText, originalUrl: item.link,
          contentHash, sourceId: source.id,
          publishedAt: item.pubDate ? new Date(item.pubDate) : new Date(),
          summary, whyItMatters, categoryId: category?.id, province, city, importanceScore,
          status: importanceScore >= 40 ? 'published' : 'draft', keywords,
        }
      })
      newArticles++
    }
    ...
    await prisma.source.update({
      where: { id: sourceId },
      data: { lastSyncAt: new Date(), articlesCount: { increment: newArticles }, errorCount: 0, lastError: null }
    })
    ...
    await prisma.source.update({ where: { id: sourceId }, data: { errorCount: { increment: 1 }, lastError: errMsg } })
    await prisma.scrapingLog.update({ where: { id: log.id }, data: { status: 'error', finishedAt: new Date(), errorMessage: errMsg } })
...
export async function scrapeAllSources(): Promise<{ total: number; newArticles: number; errors: number }> {
  ...
  let total = 0, newArticles = 0, errors = 0
